...
class LearnableGlobalLocalMultiheadAttentionSelfAttention(nn.Module):
    # So far this one is applicable to only self attention in encoder
    # This corrsponding to global-local approach
    NUM_WEIGHTS = 9
    ...
    def prepare_local_masking(self, q_left, k_left, q_right, k_right, padding_mask, bsz, **kwargs):
        ...
        left_softmax = F.softmax(self.gumbel_alpha(padding_mask) * padded_left_weights, dim=-1).type_as(
            padded_left_weights)
        right_softmax = F.softmax(self.gumbel_alpha(padding_mask) * padded_right_weights, dim=-1).type_as(
    ...
    def forward(self, query, key, value, key_padding_mask=None, incremental_state=None,
        ...
        q_left = q_left * self.scaling
        q_right = q_right * self.scaling
...
class LearnableGlobalLocalExpConMultiheadAttentionSelfAttention(LearnableGlobalLocalMultiheadAttentionSelfAttention):
    """
    ...
    def prepare_local_masking(self, q_left, k_left, q_right, k_right, padding_mask, bsz, **kwargs):
        ...
        left_softmax = F.softmax(padded_left_weights, dim=-1).type_as(padded_left_weights)
        right_softmax = F.softmax(padded_right_weights, dim=-1).type_as(padded_right_weights)
...
class LearnableGlobalHardLocalMultiheadAttentionSelfAttention(nn.Module):
    NUM_WEIGHTS = 7
    ...
    def forward(self, query, key, value, key_padding_mask=None, incremental_state=None,
        ...
        attn_weights = attn_weights * local_att_mask
...
class LearnableGlobalLocalMultiheadAttentionSelfAttentionBlock(LearnableGlobalLocalMultiheadAttentionSelfAttention):
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
                 gumbel_alpha=1.0, num_block=2):
    ...
    def prepare_local_masking(self, q_left, k_left, q_right, k_right, padding_mask, bsz, **kwargs):
        ...
        # triu = torch.ones(src_len, src_len, device=GPU, dtype=q_left.dtype).triu_().unsqueeze(0)
        triu = torch.ones(src_len, src_len, device=GPU, dtype=q_left.dtype).triu_().unsqueeze_(0)[:,
               [self.num_block * int(x / self.num_block) for x in range(src_len)], :]
        # triu = torch.ones(src_len, src_len, device=GPU, dtype=q_left.dtype).triu().unsqueeze(0)
        # triu_t = triu.transpose(1, 2)
...
class LearnableGlobalHardLocalMultiheadAttentionSelfAttentionBlock(
...
class LearnableGlobalHardLocalMultiheadAttentionSelfAttentionNormalization(
    LearnableGlobalHardLocalMultiheadAttentionSelfAttention):
    NUM_WEIGHTS = 7
    ...
    def forward(self, query, key, value, key_padding_mask=None, incremental_state=None,
        ...
        attn_weights = attn_weights * local_att_mask
        summall = attn_weights.sum(dim=-1, keepdim=True)
        attn_weights = attn_weights / summall
        # summall = attn_weights.sum(dim=-1, keepdim=True)
        # ifzero = torch.any(summall.abs() < 1e-2)
        # # assert not ifzero, f"contain zeros in sum"
        # if ifzero:
...
class LearnableGlobalHardLocalExpConMultiheadAttentionSelfAttention(
    LearnableGlobalHardLocalMultiheadAttentionSelfAttention):
    """
    This is not appliable to cross-attention!
    """
...
class LearnableGlobalHardLocalExpConMultiheadAttentionSelfAttentionNormalization(
...
class LearnableGlobalHardLocalMultiheadAttentionSelfAttentionNormalizationBlock(
    LearnableGlobalHardLocalMultiheadAttentionSelfAttentionNormalization):
    NUM_WEIGHTS = 7
...
class LearnableGlobalHardLocalMultiheadAttentionSelfAttentionAdditiveMasking(nn.Module):
    NUM_WEIGHTS = 7
    ...
    def __init__(
        ...
        # bias vector for left and right
        self.v_left = Parameter(torch.Tensor(1, self.head_dim))
        self.v_right = Parameter(torch.Tensor(1, self.head_dim))
        if bias:
            self.in_proj_bias = Parameter(torch.Tensor(self.NUM_WEIGHTS * embed_dim))
        else:
            self.register_parameter('in_proj_bias', None)
    ...
    def prepare_local_masking(self, transform_query, transform_key, padding_mask, bsz, **kwargs):
        # print(transform_query.data_ptr() == transform_key.data_ptr())
        # input()
        # assert (torch.eq(transform_query, transform_key)),f'so far we only use this for self attention'
        # left_attn_weights = torch.bmm(q_left, k_left.transpose_(1, 2))
        # right_attn_weights = torch.bmm(q_right, k_right.transpose_(1, 2))
        query_size = transform_query.size()
        # query_size: [b*h, lq, hidden_dim]
        query_len = query_size[1]
        hidden_dim = query_size[2]
        x_2 = transform_key.transpose(1, 2).unsqueeze(1).repeat(1, query_len, 1, 1)
        x_1 = x_2.transpose(1, 3)
        # print(query_size)
        # print(x_1.size())
        # print(x_2.size())
        # input()
        x_1_left = self.in_proj_local_left_q(x_1.transpose(2, 3)).transpose(2, 3)
        x_1_left = x_1_left.contiguous().view(-1, self.head_dim, query_len)
        x_2_left = self.in_proj_local_left_k(x_2.transpose(2, 3)).transpose(2, 3)
        x_2_left = x_2_left.contiguous().view(-1, self.head_dim, query_len)
        x_1_right = self.in_proj_local_right_q(x_1.transpose(2, 3)).transpose(2, 3)
        x_1_right = x_1_right.contiguous().view(-1, self.head_dim, query_len)
        x_2_right = self.in_proj_local_right_k(x_2.transpose(2, 3)).transpose(2, 3)
        x_2_right = x_2_right.contiguous().view(-1, self.head_dim, query_len)

        # print(query_size)
        # print((x_1_left+x_2_left).size())
        # print((x_1_right + x_2_right).size())
        # print(self.v_left.size())
        # input()
        left_attn_weights = torch.matmul(self.v_left, x_1_left + x_2_left)
        right_attn_weights = torch.matmul(self.v_right, x_1_right + x_2_right)
        # print(left_attn_weights.size())
        # print(right_attn_weights.size())
        # print(self.v_left.size())
        # input()
        left_attn_weights = left_attn_weights.view(-1, query_len, query_len)
        right_attn_weights = right_attn_weights.view(-1, query_len, query_len)
        ...
        # triu = torch.ones(src_len, src_len).float().cuda().triu_().unsqueeze_(0)
        triu = torch.ones(src_len, src_len, device=GPU, dtype=transform_query.dtype).triu_().unsqueeze_(0)
        # triu = torch.ones(src_len, src_len, device=GPU, dtype=q_left.dtype).triu_().unsqueeze(0)
        # triu = torch.ones(src_len, src_len, device=GPU, dtype=q_left.dtype).triu().unsqueeze(0)
        # triu_t = triu.transpose(1, 2)
    ...
    def forward(self, query, key, value, key_padding_mask=None, incremental_state=None,
        ...
        # if q_right is not None:
        #     q_right = q_right.contiguous().view(-1, bsz * self.num_heads, self.head_dim).transpose(0, 1)
        query_transform = query.transpose(0, 1)
        key_transform = key.transpose(0, 1)
        # query_transform: [b*h, l, d]
        # q: [b*h, l, d]
        # k: [b*h, l, d]
        # k_local: [b*h, l, d]
        # v: [b*h, l, d]
        ...
        # local_att_mask = self.prepare_local_masking(q_left, k_left, q_right, k_right, exp_key_padding_mask, bsz)
        local_att_mask = self.prepare_local_masking(query_transform, key_transform, exp_key_padding_mask, bsz)
...
class MultiheadAttentionCheckDecoder(nn.Module):
    NUM_WEIGHTS = 7
    ...
    def prepare_local_masking(self, q_left, k_left, q_right, k_right, padding_mask, bsz, **kwargs):
        ...
        # triu = torch.ones(src_len, src_len).float().cuda().triu_().unsqueeze_(0)
        triu = torch.ones(src_len, src_len, device=GPU, dtype=q_left.dtype).triu_().unsqueeze_(0)
        eye = torch.eye(src_len, device=GPU, dtype=q_left.dtype).unsqueeze_(0)
        future_mask = (triu - eye).byte()

        left_attn_weights = left_attn_weights.masked_fill(future_mask, float('-inf'))
        right_attn_weights = right_attn_weights.masked_fill(future_mask, float('-inf'))
        # triu = torch.ones(src_len, src_len, device=GPU, dtype=q_left.dtype).triu_().unsqueeze(0)
        # triu = torch.ones(src_len, src_len, device=GPU, dtype=q_left.dtype).triu().unsqueeze(0)
        # triu_t = triu.transpose(1, 2)
...
class LearnableGlobalLocalMultiheadAttentionDecoder(LearnableGlobalLocalMultiheadAttentionSelfAttention):
    ...
    def forward(self, query, key, value, key_padding_mask=None, incremental_state=None,
        ...
        local_att_mask = self.prepare_local_masking(q_left, k_left, q_right, k_right, attn_mask, exp_key_padding_mask,
                                                    bsz)
    ...
    def prepare_local_masking(self, q_left, k_left, q_right, k_right, attn_mask, padding_mask, bsz, **kwargs):
        # left_attn_weights = torch.bmm(q_left, k_left.transpose_(1, 2))
        # right_attn_weights = torch.bmm(q_right, k_right.transpose_(1, 2))
        ...
        right_attn_weights = torch.bmm(q_right, k_right.transpose(1, 2))
        if attn_mask is not None:
            left_attn_weights = left_attn_weights + attn_mask
            right_attn_weights = right_attn_weights + attn_mask
        # assert list(attn_weights.size()) == [bsz * self.num_heads, tgt_len, src_len]
        # weights: [b * h, lq, lk]
        left_size = left_attn_weights.size()
        # bsz = bh // self.num_heads
...
class LearnableGlobalHardLocalMultiheadAttentionDecoder(LearnableGlobalHardLocalMultiheadAttentionSelfAttention):
    def forward(self, query, key, value, key_padding_mask=None, incremental_state=None,
                need_weights=True, static_kv=False, attn_mask=None):
        """Input shape: Time x Batch x Channel
        Self-attention can be implemented by passing in the same arguments for
        ...
        attn_weights = F.softmax(attn_weights.float(), dim=-1).type_as(attn_weights).type_as(attn_weights)
        attn_weights = F.dropout(attn_weights, p=self.dropout, training=self.training)
...
class LearnableGlobalHardLocalMultiheadAttentionNormalizationDecoder(
    LearnableGlobalHardLocalMultiheadAttentionSelfAttentionNormalization):
    def prepare_local_masking(self, q_left, k_left, q_right, k_right, padding_mask, bsz, **kwargs):
        # left_attn_weights = torch.bmm(q_left, k_left.transpose_(1, 2))
        # right_attn_weights = torch.bmm(q_right, k_right.transpose_(1, 2))
...
class LearnableGlobalLocalMultiheadAttentionDecoderBlock(LearnableGlobalLocalMultiheadAttentionDecoder):
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
                 gumbel_alpha=1.0, num_block=2):
        super().__init__(embed_dim, num_heads, dropout, bias, add_bias_kv, add_zero_attn, gumbel_alpha)
        self.num_block = num_block
    ...
    def prepare_local_masking(self, q_left, k_left, q_right, k_right, padding_mask, bsz, **kwargs):
        ...
        triu_normal = torch.ones(src_len, src_len, device=GPU, dtype=q_left.dtype).triu_().unsqueeze_(0)
        eye_normal = torch.eye(src_len, device=GPU, dtype=q_left.dtype).unsqueeze_(0)
        future_mask = (triu_normal - eye_normal).byte()
        left_attn_weights = left_attn_weights.masked_fill(future_mask, float('-inf'))
        right_attn_weights = right_attn_weights.masked_fill(future_mask, float('-inf'))
...
class LearnableGlobalHardLocalMultiheadAttentionDecoderBlock(LearnableGlobalHardLocalMultiheadAttentionDecoder):
    NUM_WEIGHTS = 7
...
class LearnableGlobalHardLocalMultiheadAttentionNormalizationDecoderBlock(
...
def run_check_dot_product(gb):
    b = 10
    l = 10
    d = 20
    w_l = torch.Tensor(d, d).uniform_()
    w_r = torch.Tensor(d, d).uniform_()
    x = torch.Tensor(l, d).uniform_()
    triu = torch.ones(l, l).triu_().int()
    eye = torch.eye(l).int()
    future_mask = (triu - eye).byte()
    past_mask = future_mask.transpose(0, 1)

    x_l = torch.matmul(x, w_l)
    x_r = torch.matmul(x, w_r)
    score = torch.matmul(x_l, x_r.transpose(0, 1))
    left_attn_weights = score.masked_fill(future_mask, float('-inf'))
    right_attn_weights = score.masked_fill(past_mask, float('-inf'))
...
def run_check_additive(gb):
    b = 10
    l = 3
    d = 2
    v_a = torch.Tensor(1, d).uniform_()
    w_1 = torch.Tensor(d, d).uniform_()
    w_2 = torch.Tensor(d, d).uniform_()
    x = torch.Tensor(l, d).uniform_()
    x_2 = x.transpose(0, 1).repeat(l, 1, 1)
    x_1 = x_2.transpose(0, 2)
    attention = torch.matmul(v_a, torch.matmul(w_1, x_1) + torch.matmul(w_2, x_2))
    print('v_a')
    print(v_a)
    print('w')
    print(w_1)
    ...
    print(x_1)
    print(x_2)
    print((torch.matmul(w_1, x_1) + torch.matmul(w_2, x_2)).size())
    print('attention')
    # print(attention)
    print(attention.view(l, -1))
    print('check_result')
    print(torch.matmul(v_a, torch.matmul(w_1, x[0:1].transpose(0, 1)) + torch.matmul(w_2, x[1:2].transpose(0, 1))))
    return 0


def run_check_additive_batch(gb):
    b = 4
    l = 3
    d = 2
    v_a = torch.Tensor(1, d).uniform_()
    w_1 = torch.Tensor(d, d).uniform_()
    w_2 = torch.Tensor(d, d).uniform_()
    x = torch.Tensor(b, l, d).uniform_()
    print(x)
    x_2 = x.transpose(1, 2).unsqueeze(1).repeat(1, l, 1, 1)
    x_1 = x_2.transpose(1, 3)
    attention = torch.matmul(v_a, torch.matmul(w_1, x_1) + torch.matmul(w_2, x_2))
    print('v_a')
    print(v_a)
    ...
    print('attention')
    # print(attention)
    print(attention.view(b, l, -1))
    print('check_result')
    print(
        torch.matmul(v_a, torch.matmul(w_1, x[1][0:1].transpose(0, 1)) + torch.matmul(w_2, x[1][2:3].transpose(0, 1))))
    return 0
