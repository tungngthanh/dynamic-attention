...
perl $mosesdecoder/scripts/tokenizer/replace-unicode-punctuation.perl -l fr  < $hyp > $hyp.n

# Tokenize.
perl $mosesdecoder/scripts/tokenizer/tokenizer.perl -l fr < $hyp.n > $hyp.tok

perl $mosesdecoder/scripts/generic/multi-bleu.perl $ref < $hyp.tok


perl -ple 's{(\S)-(\S)}{$1 ##AT##-##AT## $2}g' < $hyp > $hyp.atat
